...
import { generateError, buildMoviesQuery, insidePercent, regular } from '../utils'

const movies = express.Router()

movies.get('/movies', (req, res) => {
  const query = buildMoviesQuery('SELECT * FROM movie WHERE title LIKE $1', insidePercent, req.query.search)

  if (req.query.page <= 0) {
    res.status(400).send(generateError(400, 'Page number cannot be less than 1'))
  } else if (req.query.page > 0 && req.query.search) {
    query.text += ' ORDER BY CTID ASC LIMIT 10 OFFSET $2'
    query.values.push(`${(req.query.page - 1) * 10}`)
  } else if (req.query.page > 0) {
    query.text += ' ORDER BY CTID ASC LIMIT 10 OFFSET $1'
